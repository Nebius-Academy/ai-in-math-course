...
class MDP:
    def __init__(self, transition_probs, rewards, initial_state=None, seed=None):
        ...
        :param rewards: rewards[s][a] = r(s,a)
            A dict[state -> dict] of dicts[action -> reward]
    ...
    def get_reward(self, state, action, next_state=None):
        """ return the reward you get for taking action in state """
        assert action in self.get_possible_actions(
            state), "cannot do action %s from state %s" % (action, state)
        return self._rewards.get(state, {}).get(action, 0.0)
    ...
    def _check_param_consistency(self, transition_probs, rewards):
        ...
        for state in rewards:
            ...
            for action in rewards[state]:
                assert isinstance(rewards[state][action], (int, float)), \
                    "rewards for %s, %s should be a number but is instead %s" % (
...
class FrozenLakeEnv(MDP):
    ...
    def __init__(self, desc=None, map_name="4x4", slip_chance=0.2, seed=None):
        ...
        self.trajectory = []
        self._render_q_values = None
        self._render_total_rewards = None
        self._render_episode_n = None
        self._render_visualize_every = 1
        self._render_delay = 0.0
        self._render_title = None

        nrow, ncol = desc.shape
        states = [(i, j) for i in range(nrow) for j in range(ncol)] + ["TERMINAL"]
        ...
        for state in states:
            ...
            for action_i in range(len(actions)):
                ...
                if desc[row, col] == "H":
                    ...
                    continue
                if desc[row, col] == "G":
                    transition_probs[state][action]["TERMINAL"] = 1.0
                ...
                for movement_i in [(action_i - 1) % len(actions), action_i,
                    ...
                    if prob == 0:
                        continue
                    if (newrow, newcol) not in transition_probs[state][
                            action]:
                        transition_probs[state][action][
                            newrow, newcol] = prob
                    else:
    ...
    def configure_training_visualization(self, q_values, total_rewards, episode_n, visualize_every=100, delay=0.01):
        self._render_q_values = q_values
        self._render_total_rewards = total_rewards
        self._render_episode_n = episode_n
        self._render_visualize_every = visualize_every
        self._render_delay = delay
    ...
    def start_episode(self, episode_index):
        ...
        if isinstance(self._current_state, tuple):
            self.trajectory = [self._current_state]
        else:
            self.trajectory = []
        self.lastaction = None
        self._last_state = None
    ...
    def step(self, action):
        ...
        next_state, reward, is_done, info = MDP.step(self, action)

        if isinstance(next_state, tuple):
            tile = self.desc[next_state]
            if tile == "H":
                is_done = True
            elif tile == "G":
                reward = 1.0
        ...
        if not hasattr(self, "trajectory"):
            self.trajectory = []
        if len(self.trajectory) == 0 and isinstance(self._last_state, tuple):
            self.trajectory.append(self._last_state)
        self.trajectory.append(next_state)

        return next_state, reward, is_done, info

    def render(self):
        if self._render_q_values is None or self._render_total_rewards is None or self._render_episode_n is None:
        ...
        should_visualize = self._episode_index % self._render_visualize_every == 0
        if not should_visualize:
            return

        import importlib
        import env_frozen_lake_visualize

        importlib.reload(env_frozen_lake_visualize)
        env_frozen_lake_training_visualize = env_frozen_lake_visualize.env_frozen_lake_training_visualize

        rewards_history = list(self._render_total_rewards) + [self._episode_reward]
        env_frozen_lake_training_visualize(
            self,
            self._render_q_values,
            rewards_history,
            self._render_episode_n,
            title=f"Episode {self._episode_index}, step {self._episode_step}",
            trajectory=self.trajectory,
            agent_state=self._current_state if isinstance(self._current_state, tuple) else None,
            active_state=self._last_state,
            selected_action=self.lastaction,
            updated_state=self._last_state,
            updated_action=self.lastaction,
        )
